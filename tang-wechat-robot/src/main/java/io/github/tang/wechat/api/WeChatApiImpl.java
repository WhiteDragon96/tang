...
import io.github.tang.wechat.robot.LoginStatus;
import io.github.tang.wechat.robot.WeChatBot;
import io.github.tang.wechat.api.client.BotClient;
import io.github.tang.wechat.api.constant.Constant;
import io.github.tang.wechat.api.constant.StateCode;
import io.github.tang.wechat.api.enums.AccountType;
import io.github.tang.wechat.api.enums.ApiURL;
import io.github.tang.wechat.api.enums.RetCode;
import io.github.tang.wechat.api.model.Account;
import io.github.tang.wechat.api.model.DownLoad;
import io.github.tang.wechat.api.model.HotReload;
import io.github.tang.wechat.api.model.LoginSession;
import io.github.tang.wechat.api.model.Message;
import io.github.tang.wechat.api.model.Recommend;
import io.github.tang.wechat.api.model.SendMessage;
...
import io.github.tang.wechat.api.request.BaseRequest;
import io.github.tang.wechat.api.request.FileRequest;
import io.github.tang.wechat.api.request.JsonRequest;
...
import io.github.tang.wechat.api.response.MediaResponse;
import io.github.tang.wechat.api.response.WebInitResponse;
import io.github.tang.wechat.api.response.WebSyncResponse;
import io.github.tang.wechat.exception.WeChatException;
import io.github.tang.wechat.utils.*;
...
public class WeChatApiImpl implements WeChatApi {
	...
	private static final Pattern UUID_PATTERN = Pattern
			.compile("window.QRLogin.code = (\\d+); window.QRLogin.uuid = \"(\\S+?)\";");
	...
	private static final Pattern SYNC_CHECK_PATTERN = Pattern
	 ...
	 */
	private void autoLogin(LoginStatus loginStatus) throws FileNotFoundException {
		String file = bot.config().assetsDir() + "/" + bot.config().weixinId() + ".json";
		HotReload hotReload = WeChatUtils.fromJson(new FileReader(file), HotReload.class);
		HotReload.reLogin(bot, hotReload);
	}
	...
	public void relogin() {
		...
		login(true, this.bot.getLoginStatus());
	}

	@Override
	public void login(boolean autoLogin, LoginStatus loginStatus) {
		...
		if (autoLogin) {
			try {
				this.autoLogin(loginStatus);
			} catch (FileNotFoundException e) {
				autoError = true;
			}
		}

		if (autoError || !autoLogin) {
			this.logging = true;
			while (logging) {
				this.uuid = pushLogin();
				if (null == this.uuid) {
					...
					log.info("开始下载二维码");
					loginStatus.setMessage("开始下载二维码");
					this.getQrImage(this.uuid, bot.config().qrPath(), bot.config().showTerminal());
					log.info("请使用微信扫描二维码");
					loginStatus.setQrVersion(loginStatus.getQrVersion() + 1);
					loginStatus.setMessage("请使用微信扫描二维码");
				}
				...
				while (null == isLoggedIn || !isLoggedIn) {
					...
					if (StateCode.SUCCESS.equals(status)) {
						...
						if (null != isLoggedIn) {
							loginStatus.setMessage("请在手机上确认登录");
							log.info("请在手机上确认登录");
							isLoggedIn = null;
						}
						...
						// 二维码超时了 ， 判断为退出了
						loginStatus.setStatus(2);// 判断为已经退出
						return ;
					} else {
						break ;
					}
				...
				if (logging) {
					...
					loginStatus.setMessage("登录超时，重新加载二维码");
					loginStatus.setQrVersion(loginStatus.getQrVersion() + 1);
				}
			}
		}
		if (!this.webInit()) {
			HotReload.clearLogin(bot);
			this.login(false, loginStatus);
			return;
		}
		this.statusNotify();
		this.loadContact(0);
		...
		loginStatus.setMessage(String.format("应有%d个联系人，读取到联系人%d个", this.memberCount, this.accountMap.size()));		

		log.info("共有 {} 个群 | {} 个直接联系人 | {} 个特殊账号 ｜ {} 公众号或服务号", this.groupUserNames.size(), this.contactList.size(),
				this.specialUsersList.size(), this.publicUsersList.size());
		loginStatus.setMessage(String.format("共有%d个群 |%d个直接联系人 |%d个特殊账号 ｜%d公众号", this.groupUserNames.size(), this.contactList.size(),
				this.specialUsersList.size(), this.publicUsersList.size()));		
		// 加载群聊信息，群成员
		this.loadGroupList();

		log.info("[{}] 登录成功.", bot.session().getNickName());
		loginStatus.setMessage(String.format("[%s] 登录成功.", bot.session().getNickName()));	
		this.startRevive();
		this.logging = false;
		loginStatus.setStatus(1);
		
		this.bot.saveAllAccount(this.contactList);
	}
	...
	private String getUUID() {
		...
		ApiResponse response = this.client.send(new StringRequest("https://login.weixin.qq.com/jslogin")
				.add("appid", "wx782c26e4c19acffb").add("fun", "new").add("redirect_uri", "https://wx.qq.com/cgi-bin/mmwebwx-bin/webwxnewloginpage?mod=desktop").add("lang" ,"zh_CN"));
	 ...
	 *            是否在终端显示输出
	 */
	private void getQrImage(String uuid, String qrPath, boolean terminalShow) {
		String uid = null != uuid ? uuid : this.uuid;
		String imgDir = bot.config().assetsDir();

		FileResponse fileResponse = this.client
				.download(new FileRequest(String.format("%s/qrcode/%s", Constant.BASE_URL, uid)));

		InputStream inputStream = fileResponse.getInputStream();
		if (qrPath != null && qrPath.length() > 0) {
			File tmpFile = new File(qrPath);
			WeChatUtils.saveFile(inputStream, tmpFile.getParent(), tmpFile.getName());
		} else {
			File qrCode = WeChatUtils.saveFile(inputStream, imgDir, bot.config().weixinId() + "qrcode.png");
			DateUtils.sleep(200);
			try {
				QRCodeUtils.showQrCode(qrCode, terminalShow);
			} catch (Exception e) {
				this.getQrImage(uid, qrPath, terminalShow);
			}
		}
	}
	...
	private String checkLogin(String uuid) {
		String uid = null != uuid ? uuid : this.uuid;
		String url = String.format("%s/cgi-bin/mmwebwx-bin/login", Constant.BASE_LOGIN_URL);
		long time = System.currentTimeMillis();
		ApiResponse response = this.client.send(new StringRequest(url).add("loginicon", true).add("uuid", uid)
				.add("tip", "1").add("_", time).add("r", ~time).timeout(30));
//		.add("tip", "1").add("_", time).add("r", (int) ~time).timeout(30));
	...
	private boolean processLoginSession(String loginContent) {
		...
		ApiResponse response = this.client.send(new StringRequest(loginSession.getUrl()+ "&fun=new&version=v2&mod=desktop&lang=zh_CN")
		        .header("client-version", Constant.UOS_PATCH_CLIENT_VERSION)//
		        .header("extspam", Constant.UOS_PATCH_EXTSPAM)//
	...
	private void statusNotify() {
		...
		String url = String.format("%s/webwxstatusnotify?lang=zh_CN&pass_ticket=%s", bot.session().getUrl(),
				bot.session().getPassTicket());

		this.client.send(new JsonRequest(url).post().jsonBody().add("BaseRequest", bot.session().getBaseRequest())
				.add("Code", 3).add("FromUserName", bot.session().getUserName())
				.add("ToUserName", bot.session().getUserName()).add("ClientMsgId", System.currentTimeMillis() / 1000));
	}
	...
	private boolean webInit() {
		...
		String url = String.format("%s/webwxinit?r=%d&pass_ticket=%s", bot.session().getUrl(), r,
				bot.session().getPassTicket());

		JsonResponse response = this.client
				.send(new JsonRequest(url).post().jsonBody().add("BaseRequest", bot.session().getBaseRequest()));
		...
		if (StringUtils.isEmpty(account.getNickName())) {// 登录失败
			return false;
		}
	...
	public SyncCheckRet syncCheck() {
		...
		try {
			ApiResponse response = this.client.send(new StringRequest(url).add("r", System.currentTimeMillis())
					.add("skey", bot.session().getSKey()).add("sid", bot.session().getWxSid())
					.add("uin", bot.session().getWxUin()).add("deviceid", bot.session().getDeviceId())
					.add("synckey", bot.session().getSyncKeyStr()).add("_", System.currentTimeMillis()).timeout(30));
			...
			if (matcher.find()) {
				...
				return new SyncCheckRet(RetCode.parse(Integer.valueOf(matcher.group(1))),
	...
	public WebSyncResponse webSync() {
		String url = String.format("%s/webwxsync?sid=%s&sKey=%s&passTicket=%s", bot.session().getUrl(),
				bot.session().getWxSid(), bot.session().getSKey(), bot.session().getPassTicket());

		JsonResponse response = this.client
				.send(new JsonRequest(url).post().jsonBody().add("BaseRequest", bot.session().getBaseRequest())
						.add("SyncKey", bot.session().getSyncKey()).add("rr", ~(System.currentTimeMillis() / 1000)));
		...
		if (webSyncResponse.getSyncCheckKey() != null && webSyncResponse.getSyncCheckKey().getCount() > 0) {
			bot.session().setSyncKey(webSyncResponse.getSyncCheckKey());
		} else {
	...
	public void logout() {
		if (bot.isRunning()) {
			...
			this.client.send(
					new StringRequest(url).add("redirect", 1).add("type", 1).add("sKey", bot.session().getSKey()));
		}
		this.logging = false;
		this.client.cookies().clear();
		bot.getLoginStatus().setStatus(2);// 标记已经退出
		String file = bot.config().assetsDir() + "/" + bot.config().weixinId() + ".json";
	...
	public void loadContact(int seq) {
		log.info("开始获取联系人信息");
		while (true) {
			String url = String.format("%s/webwxgetcontact?r=%s&seq=%s&skey=%s", bot.session().getUrl(),
					System.currentTimeMillis(), seq, bot.session().getSKey());

			JsonResponse response = this.client.send(new JsonRequest(url).timeout(30).jsonBody());
			...
			List<Account> memberList = WeChatUtils.fromJson(WeChatUtils.toJson(jsonObject.getAsJsonArray("MemberList")),
	...
	public void loadGroupList() {
		...
		String url = String.format("%s/webwxbatchgetcontact?type=ex&r=%s&pass_ticket=%s", bot.session().getUrl(),
				System.currentTimeMillis() / 1000, bot.session().getPassTicket());

		// 加载群信息 群成员
		JsonResponse jsonResponse = this.client
				.send(new JsonRequest(url).post().jsonBody().add("BaseRequest", bot.session().getBaseRequest())
						.add("Count", groupUserNames.size()).add("List", list));
//		botService.saveGroupUser(jsonResponse);
		this.groupList = WeChatUtils.fromJson(
				WeChatUtils.toJson(jsonResponse.toJsonObject().getAsJsonArray("ContactList")),
				new TypeToken<List<Account>>() {
				});
	}
	...
	public boolean revokeMsg(String msgId, String toUser) {
		String url = String.format("%s/webwxrevokemsg?pass_ticket=%s&r=%s", bot.session().getUrl(),
				bot.session().getPassTicket(), System.currentTimeMillis() / 1000);

		JsonResponse response = client.send(new JsonRequest(url).post().jsonBody().add("SvrMsgId", msgId)
				.add("ToUserName", toUser).add("ClientMsgId", System.currentTimeMillis())
	...
	public boolean verify(Recommend recommend) {
		String url = String.format("%s/webwxverifyuser?r=%s&lang=zh_CN&pass_ticket=%s", bot.session().getUrl(),
				System.currentTimeMillis() / 1000, bot.session().getPassTicket());
		...
		JsonResponse response = client.send(new JsonRequest(url).post().jsonBody()
				.add("BaseRequest", bot.session().getBaseRequest()).add("Opcode", 3).add("VerifyUserListSize", 1)
				.add("VerifyUserList", verifyUserList).add("VerifyContent", "").add("SceneListCount", 1)
				.add("SceneList", Arrays.asList(33)).add("skey", bot.session().getSyncKeyStr()));
		boolean re = null != response && response.success();
		return re;
	}
	...
	public boolean addFriend(String friendUserName, String msg) {
		...
		JsonResponse response = client.send(new JsonRequest(url).post().jsonBody()
				.add("BaseRequest", bot.session().getBaseRequest()).add("Opcode", 2).add("VerifyUserListSize", 1)
				.add("VerifyUserList", verifyUserList).add("VerifyContent", msg).add("SceneListCount", 1)
				.add("SceneList", Arrays.asList(33)).add("skey", bot.session().getSyncKeyStr()));
		return null != response && response.success();
	}
	...
	public boolean createChatRoom(String topic, List<String> members) {
		...
		JsonResponse response = client.send(new JsonRequest(url).post().jsonBody().add("MemberCount", members.size())
				.add("MemberList", memberList).add("Topic", topic).add("BaseRequest", bot.session().getBaseRequest()));
		return null != response && response.success();
	}
	...
	public boolean removeMemberByGroup(String member, String group) {
		...
		JsonResponse response = client.send(new JsonRequest(url).post().jsonBody().add("DelMemberList", member)
				.add("ChatRoomName", group).add("BaseRequest", bot.session().getBaseRequest()));
		return null != response && response.success();
	}
	...
	public boolean inviteJoinGroup(String member, String group) {
		...
		JsonResponse response = client.send(new JsonRequest(url).post().jsonBody().add("AddMemberList", member)
				.add("ChatRoomName", group).add("BaseRequest", bot.session().getBaseRequest()));
		return null != response && response.success();
	}
	...
	public boolean modifyGroupName(String oldTopic, String newTopic) {
		...
		JsonResponse response = client.send(new JsonRequest(url).post().jsonBody().add("NewTopic", newTopic)
				.add("ChatRoomName", account.getUserName()).add("BaseRequest", bot.session().getBaseRequest()));
		return null != response && response.success();
	}

	/**
	 * CmdId : 2 RemarkName : "小小小" UserName :
	 * "@9bf91e1b143645dd694c8e367b2c204d3758268b073468b504740841e77f40bd"
	 * 
	 * @param UserName
	 * @param remarkName
	 * @return
	 */
	@Override
	public boolean modifyRmarkName(String UserName, String remarkName) {
		String url = String.format("%s/webwxoplog", bot.session().getUrl());
		JsonResponse response = client.send(new JsonRequest(url).post().jsonBody()//
				.add("CmdId", 2)//
		...
		// 重置一下用户集合中的内容
		if (re) {
			Account acc = this.getAccountMap().get(UserName);
			acc.setRemarkName(remarkName);
		}
		return re;
	...
	private String getUserRemarkName(String id) {
		...
		String nickName = StringUtils.isNotEmpty(account.getRemarkName()) ? account.getRemarkName()
				: account.getNickName();
	...
	private List<Account> getNameById(String id) {
		...
		JsonResponse response = this.client
				.send(new JsonRequest(url).post().jsonBody().add("BaseRequest", bot.session().getBaseRequest())
						.add("Count", bot.session().getBaseRequest()).add("List", list));

		return WeChatUtils.fromJson(WeChatUtils.toJson(response.toJsonObject().getAsJsonObject("")),
				new TypeToken<List<Account>>() {
				});
	}
	...
	private WeChatMessage processMsg(Message message) {
		...
		if (message.isGroup()) {
			...
			if (message.getToUserName().contains(Constant.GROUP_IDENTIFY) && !groupUserNames.contains(message.getToUserName())) {
				this.groupUserNames.add(message.getToUserName());
			}
			if (content.contains(Constant.GROUP_BR)) {
				int index = content.indexOf(Constant.GROUP_BR);
				groupUserName = content.substring(0, index);
		...
		WeChatMessage.WeChatMessageBuilder weChatMessageBuilder = WeChatMessage.builder().raw(message)
				.id(message.getId()).fromUserName(message.getFromUserName()).toUserName(message.getToUserName())
				.mineUserName(bot.session().getUserName()).mineNickName(bot.session().getNickName())//
				.robotUserName(bot.config().weixinId()).isMe(message.isMe()).fromGroupUserName(groupUserName)//
				.msgType(message.msgType()).text(content);
		...
		// 好友请求
		case ADD_FRIEND:
			weChatMessageBuilder.recommend(message.getRecommend());
	...
	private String downloadImg(String msgId) {
		return this.downloadFile(new DownLoad(ApiURL.IMAGE, bot.session().getUrl(), msgId, bot.session().getSKey())
	...
	private String downloadIconImg(String msgId) {
		return this.downloadFile(
				new DownLoad(ApiURL.ICON, bot.session().getUrl(), msgId, bot.session().getSKey()).msgId(msgId));
	}
	...
	private String downloadHeadImg(String userName) {
		return this
				.downloadFile(new DownLoad(ApiURL.HEAD_IMG, bot.session().getUrl(), userName, bot.session().getSKey())
						.msgId(userName));
	}
	...
	private String downloadVideo(String msgId) {
		return this.downloadFile(new DownLoad(ApiURL.VIDEO, bot.session().getUrl(), msgId, bot.session().getSKey())
				.msgId(msgId).suffix(".mp4").saveByDay());
	}
	...
	private String downloadVoice(String msgId) {
		return this.downloadFile(new DownLoad(ApiURL.VOICE, bot.session().getUrl(), msgId, bot.session().getSKey())
				.msgId(msgId).suffix(".mp3").saveByDay());
	}
	...
	public MediaResponse uploadMedia(String toUser, String filePath) {
		...
		ApiResponse response = this.client.send(new StringRequest(url).post().multipart().fileName(file.getName())
				.add("id", "WU_FILE_0").add("name", filePath).add("type", mimeType)
				.add("lastModifieDate", new SimpleDateFormat("yyyy MM dd HH:mm:ss").format(new Date()))
				.add("size", String.valueOf(size)).add("mediatype", mediatype)
				.add("uploadmediarequest", WeChatUtils.toJson(uploadMediaRequest)).add("webwx_data_ticket", dataTicket)
	...
	public boolean sendImg(String toUserName, String filePath) {
		...
		String url = String.format("%s/webwxsendmsgimg?fun=async&f=json&pass_ticket=%s", bot.session().getUrl(),
				bot.session().getPassTicket());
		...
		JsonResponse response = this.client.send(new JsonRequest(url).post().jsonBody()
				.add("BaseRequest", bot.session().getBaseRequest()).add("Msg", msg));
		return null != response && response.success();
	}
	...
	public boolean sendText(String toUserName, String msg) {
		DateUtils.sendSleep();
		String url = String.format("%s/webwxsendmsg?pass_ticket=%s", bot.session().getUrl(),
				bot.session().getPassTicket());
		String msgId = System.currentTimeMillis() / 1000 + StringUtils.random(6);
	...
	@Override
	public void getBotApi(String url){
		JsonResponse response = this.client
				.send(new JsonRequest(url).post().jsonBody());
	...
	public boolean sendFile(String toUser, String filePath) {
		...
		String url = String.format("%s/webwxsendappmsg?fun=async&f=json&pass_ticket=%s", bot.session().getUrl(),
				bot.session().getPassTicket());
		...
		JsonResponse response = client.send(new JsonRequest(url).post().jsonBody()
				.add("BaseRequest", bot.session().getBaseRequest()).add("Msg", msgMap).add("Scene", 0));
		return null != response && response.success();
	}
	 ...
	 */
	@Override
	public void addAccount(List<Account> modContactList) {
		for (Account ac : modContactList) {
			Account oldAc = this.getAccountById(ac.getUserName());
			if (oldAc == null) {
				ac.setIsNew(true);
				contactList.add(ac);
			}
			...
			if(ac.getIsNew()) {
				try {
					String format = bot.getRemarkFormat();
					if(!StringUtils.isBlank(format)) {
						modifyRmarkName(ac.getUserName(), DateFormatUtils.format(new Date(), format));
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
	 ...
	 */
	@Override
	public void addGroupAccount(List<Account> modChatRoomMemberList) {
		for (Account ac : modChatRoomMemberList) {
			Account oldAc = this.getAccountById(ac.getUserName());
			if (oldAc == null) {
				ac.setIsNew(true);
				groupList.add(ac);

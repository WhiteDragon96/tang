...
import io.github.tang.wechat.api.WeChatApi;
...
import io.github.tang.wechat.api.model.Invoke;
...
import io.github.tang.wechat.utils.OkHttpUtils;
import io.github.tang.wechat.utils.WeChatUtils;
...
public class WeChatBot {
	 ...
	 * 启动微信监听
	 */
	public void start(WeChatApi weChatApi) {
		this.api = weChatApi;
		log.info("wechat-bot: {}", Constant.VERSION);
		api.login(config.autoLogin(), this.loginStatus);
	...
	public void updateLastCheck() {
		...
		if (this.config().autoLogin()) {
			String file = this.config().assetsDir() + "/" + config.weixinId() + ".json";
			File fParent = new File(file).getParentFile();
			if (!fParent.exists()) {
				fParent.mkdirs();
			}
			WeChatUtils.writeJson(file, HotReload.build(this.session(), this.botClient));
